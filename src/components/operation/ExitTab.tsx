      ...
      try {
        ...
        if (match) {
          ...
          toast({
            title: 'No reconocido',
            variant: 'destructive',
            description: 'No se detectó rostro o no coincide con ninguna persona dentro.'
          });
        }
      } catch (err: any) {
        console.error('ExitTab: Scan error:', err);
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <Button
        ...
        className="w-full h-20 text-lg font-medium bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white rounded-2xl mb-4 flex items-center justify-center gap-3"
      >
        <Camera className="w-8 h-8" />
        <span>Escanear Salida</span>
      </Button>
